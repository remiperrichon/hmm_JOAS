...
Flight2$smoothed_Altitude_rate0_001 = ksmooth(x=Flight2$time01, y=Flight2$Altitude_rate, "normal", bandwidth = 0.001)$y
Flight2$smoothed_Altitude_rate0_01 = ksmooth(x=Flight2$time01, y=Flight2$Altitude_rate, "normal", bandwidth = 0.01)$y
Flight2$smoothed_Altitude_rate0_02 = ksmooth(x=Flight2$time01, y=Flight2$Altitude_rate, "normal", bandwidth = 0.02)$y
Flight2$smoothed_Altitude_rate0_03 = ksmooth(x=Flight2$time01, y=Flight2$Altitude_rate, "normal", bandwidth = 0.03)$y

Flight2$smoothed_Ground_speed0_001 = ksmooth(x=Flight2$time01, y=Flight2$Ground_speed, "normal", bandwidth = 0.001)$y
Flight2$smoothed_Ground_speed0_01 = ksmooth(x=Flight2$time01, y=Flight2$Ground_speed, "normal", bandwidth = 0.01)$y
Flight2$smoothed_Ground_speed0_05 = ksmooth(x=Flight2$time01, y=Flight2$Ground_speed, "normal", bandwidth = 0.05)$y
Flight2$smoothed_Ground_speed0_1 = ksmooth(x=Flight2$time01, y=Flight2$Ground_speed, "normal", bandwidth = 0.1)$y

Smooth_AR = ggplot(Flight2, aes(x = time01, y = Altitude_rate)) +
  geom_line(color = "red") + 
  geom_line(mapping = aes(x = time01, y = smoothed_Altitude_rate0_001), color = "#5f1560")+
  geom_line(mapping = aes(x = time01, y = smoothed_Altitude_rate0_01), color = "#7b1b7c")+
  geom_line(mapping = aes(x = time01, y = smoothed_Altitude_rate0_02), color = "#a525a6")+
  geom_line(mapping = aes(x = time01, y = smoothed_Altitude_rate0_03), color = "#c82dca")+
  theme_bw() + ylab("Altitude rate (ft/min)") + xlab("Scaled time")+
  ggtitle("Original flight")+theme(plot.title = element_text(face = "bold"))

Smooth_GS = ggplot(Flight2, aes(x = time01, y = Ground_speed)) +
  geom_line(color = "red") + 
  geom_line(mapping = aes(x = time01, y = smoothed_Ground_speed0_001), color = "#120c71")+
  geom_line(mapping = aes(x = time01, y = smoothed_Ground_speed0_01), color = "#285995")+
  geom_line(mapping = aes(x = time01, y = smoothed_Ground_speed0_05), color = "#498baf")+
  geom_line(mapping = aes(x = time01, y = smoothed_Ground_speed0_1), color = "#8ae8f5")+
  theme_bw() + ylab("Ground speed (ft/min") + xlab("Scaled time")+
  ggtitle("Original flight")+theme(plot.title = element_text(face = "bold"))
...
mllk<-function(theta.star,x){
  ...
  ind = which(!is.na(x$smoothed_Altitude_rate0_01))
  for (j in 1:3){
    allprobs[ind,j] <- dnorm(x$smoothed_Altitude_rate0_01[ind], mean = mu[j], sd = sigma[j]) 
  }
...
for (l in 1:nrow(Flight2)){
  if(Flight2$smoothed_Altitude_rate0_01[l] <= (0+epsilon) & Flight2$smoothed_Altitude_rate0_01[l] >= (0-epsilon)){
    naive_phase = c(naive_phase, "Cruise")
  }
  if(Flight2$smoothed_Altitude_rate0_01[l] > (0+epsilon)){
...
for(i in 1:nrow(Flight2)){
  ...
  Phat = NA
  for(j in 1:5){
    Sgnd[j,] = min(min(Hgnd(Flight2$Altitude[i]), Vlo(Flight2$smoothed_Ground_speed0_01[i]), RoC0(Flight2$smoothed_Altitude_rate0_01[i])), Pgnd(j))
    Sclb[j,] = min(min(Hlo(Flight2$Altitude[i]), Vmid(Flight2$smoothed_Ground_speed0_01[i]), RoCplus(Flight2$smoothed_Altitude_rate0_01[i])), Pclb(j))
    Scru[j,] = min(min(Hhi(Flight2$Altitude[i]), Vhi(Flight2$smoothed_Ground_speed0_01[i]), RoC0(Flight2$smoothed_Altitude_rate0_01[i])), Pcru(j))
    Sdes[j,] = min(min(Hlo(Flight2$Altitude[i]), Vmid(Flight2$smoothed_Ground_speed0_01[i]), RoCminus(Flight2$smoothed_Altitude_rate0_01[i])), Pdes(j))
    Slvl[j,] = min(min(Hlo(Flight2$Altitude[i]), Vmid(Flight2$smoothed_Ground_speed0_01[i]), RoC0(Flight2$smoothed_Altitude_rate0_01[i])), Plvl(j))  
