...
dist_truth_naive_climb = c()
dist_truth_fuzzy_climb = c()

dist_truth_HMM_cruise = c()
dist_truth_naive_cruise = c()
dist_truth_fuzzy_cruise = c()

dist_truth_HMM_approach = c()
dist_truth_naive_approach = c()
dist_truth_fuzzy_approach = c()

seq_bw_ar = c(0, 0.001, 0.01, 0.02, 0.03)
seq_bw_gs = c(0, 0.001, 0.01, 0.05, 0.1)
...
for(loop in 1:length(my_list_matlab)){
  ...
  df_1 = data.frame()
  for(bw_ar in 1:length(seq_bw_ar)){
    for(bw_gs in 1:length(seq_bw_gs)){

      smoothed_Altitude_rate = c()
      smoothed_Ground_speed = c()
      if(seq_bw_ar[bw_ar]==0){smoothed_Altitude_rate = Flight2$Altitude_rate}
      if(seq_bw_gs[bw_gs]==0){smoothed_Ground_speed = Flight2$Ground_speed}
      if(seq_bw_ar[bw_ar]!=0){smoothed_Altitude_rate = ksmooth(x=Flight2$time01, y=Flight2$Altitude_rate, "normal", bandwidth = seq_bw_ar[bw_ar])$y}
      if(seq_bw_gs[bw_gs]!=0){smoothed_Ground_speed = ksmooth(x=Flight2$time01, y=Flight2$Ground_speed, "normal", bandwidth = seq_bw_gs[bw_gs])$y}
      
      Flight2$smoothed_Altitude_rate = smoothed_Altitude_rate
      Flight2$smoothed_Ground_speed = smoothed_Ground_speed
...
print(seq_bw_ar[bw_ar])
print(seq_bw_ar[bw_gs])
print(dist_truth_HMM)
print(substr(my_list_matlab_short[loop], 1, nchar(my_list_matlab_short[loop]) - 4))
...
df_0 = data.frame(flight_id = rep(substr(my_list_matlab_short[loop], 1, nchar(my_list_matlab_short[loop]) - 4), 3),
                  bw_ar = rep(seq_bw_ar[bw_ar], 3), 
                  bw_gs = rep(seq_bw_ar[bw_gs], 3), 
